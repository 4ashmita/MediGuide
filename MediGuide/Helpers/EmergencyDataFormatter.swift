...
enum EmergencyDataFormatter {
    ...
    // MARK: - Blood type SMS (compact)

    static func smsBloodTypeLine(_ type: BloodType) -> String {
        "Blood type: \(type.rawValue)"
    }

    // MARK: - Blood type dispatcher screen (with universal donor/recipient note)

    static func dispatcherBloodTypeText(_ type: BloodType) -> String {
        switch type {
        case .oNegative:  return "Blood type: O- (Universal donor — can donate to anyone)"
        case .abPositive: return "Blood type: AB+ (Universal recipient — can receive any type)"
        case .unknown:    return "Blood type: Unknown — use O negative"
        default:          return "Blood type: \(type.rawValue)"
        }
    }

    // MARK: - Blood type voice readout

    static func voiceBloodTypeReadout(_ type: BloodType) -> String {
        type == .unknown ? "Blood type is unknown." : "Blood type is \(type.rawValue)."
    }
