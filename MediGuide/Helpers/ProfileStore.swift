...
enum ProfileStore {
    ...
    static func load(id: UUID) -> UserProfile? {
        // Defense-in-depth: even if the UI gate is bypassed, refuse sensitive data without auth
        guard !AuthStateManager.shared.requiresAuthentication else { return nil }
    ...
    private static func saveNonSensitive(_ profile: UserProfile) {
        ...
        if let relationship = profile.relationship {
            set(relationship, key: StorageKeys.Defaults.relationship(for: id))
        }
    }
    ...
    private static func loadNonSensitive(id: UUID) -> NonSensitive? {
        ...
        let relationship = get(ProfileRelationship.self, key: StorageKeys.Defaults.relationship(for: id))
        return (displayName, dateOfBirth, sex, bloodType, dateCreated, dateModified, lastUsed, conditionsSummary, relationship)
    }
    ...
    private static func removeNonSensitive(id: UUID) {
        ...
        ud.removeObject(forKey: StorageKeys.Defaults.relationship(for: id))
    }
    ...
    private static func assemble(id: UUID, non: NonSensitive, sensitive: SensitiveProfileData) -> UserProfile {
        ...
        profile.relationship = non.relationship
        return profile
    }
