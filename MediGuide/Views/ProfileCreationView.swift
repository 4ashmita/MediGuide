...
struct ProfileCreationView: View {
            ...
            if !vm.isForSelf {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    ScrollView(.horizontal, showsIndicators: false) {
                        HStack(spacing: 8) {
                            ForEach(ProfileRelationship.allCases, id: \.self) { rel in
                                Button(action: {
                                    vm.relationship = vm.relationship == rel ? nil : rel
                                }) {
                                    Text(rel.rawValue)
                                        .font(.caption)
                                        .fontWeight(vm.relationship == rel ? .semibold : .regular)
                                        .padding(.horizontal, 10)
                                        .padding(.vertical, 6)
                                        .background(vm.relationship == rel ? Color.red.opacity(0.12) : Color.gray.opacity(0.1), in: Capsule())
                                        .foregroundStyle(vm.relationship == rel ? .red : .primary)
                                        .overlay(Capsule().stroke(vm.relationship == rel ? Color.red : Color.clear, lineWidth: 1))
                                }
                                .buttonStyle(.plain)
                            }
                        }
        ...
        VStack(alignment: .leading, spacing: 20) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                DatePicker("", selection: $vm.dateOfBirth, in: ...Date(), displayedComponents: .date)
                    ...
                    .labelsHidden()
                ValidationErrorView(failures: vm.failuresFor(.dateOfBirth))
            }
            VStack(alignment: .leading, spacing: 6) {
                Text("Biological sex")
                    .font(.subheadline)
    ...
    private var step3: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            ValidationErrorView(failures: vm.failuresFor(.trimester))
        }
    }
    ...
    private var step5: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            .frame(minHeight: 220)
            ValidationErrorView(failures: vm.epiPenWarnings)
        }
    }
